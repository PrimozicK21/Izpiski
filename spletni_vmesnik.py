...
# POMOZNE FUNKCIJE______________________________________________________________________________________________
...
def gumb_registracija():
    ...
    if geslo_v_cistopisu_1 != geslo_v_cistopisu_2:
        return bottle.template(
            "registracija_obvestilo.html", obvestilo="Gesli se ne ujemata"
        )
    with open(skrivna_datoteka, encoding="utf-8") as dat:
        slovar = json.load(dat)
    if uporabnisko_ime in slovar:
        return bottle.template(
            "registracija_obvestilo.html",
            obvestilo="To ime je že uporabljeno. Poizkusite drugo!",
        )
    else:
        slovar[uporabnisko_ime] = (
            zasifriraj_geslo(geslo_v_cistopisu_1, str(cas).encode("utf-8")),
            cas,
        )
...
def prijava_post():
    ...
    if (
        zasifriraj_geslo(geslo_v_cistopisu, str(cas).encode("utf-8"))
        == zasifrirano_geslo
    ):
        bottle.response.set_cookie(
            "uporabnisko_ime", uporabnisko_ime, path="/", secret=SKRIVNOST
...
def stran_predmeta(id_predmeta):
    ...
    return bottle.template(
        "stran_predmeta.html",
        ucilnica=ucilnica,
        predmet=ucilnica.seznam[id_predmeta],
...
def stran_poglavja(id_predmeta, id_poglavja):
    ...
    return bottle.template(
        ...
        ucilnica=ucilnica,
        predmet=ucilnica.seznam[id_predmeta],
        poglavje=ucilnica.seznam[id_predmeta].seznam[id_poglavja],
    )
...
def pokazi_odgovor(id_predmeta, id_poglavja, id_alineje):
    ...
    return bottle.template(
        ...
        predmet=ucilnica.seznam[id_predmeta],
        poglavje=ucilnica.seznam[id_predmeta].seznam[id_poglavja],
        prikazana_alineja=ucilnica.seznam[id_predmeta]
        .seznam[id_poglavja]
        .seznam[id_alineje],
    )
...
def znam(id_predmeta, id_poglavja, id_alineje):
    ...
    ime_datoteke = ime_uporabnikove_datoteke(uporabnisko_ime)
    ucilnica = ucilnica_trenutnega_uporabnika()
    ucilnica.seznam[id_predmeta].seznam[id_poglavja].seznam[
        id_alineje
    ].spremeni_stanje_opravljenega(True)
    ucilnica.shrani_v_datoteko(ime_datoteke)
...
def ne_znam(id_predmeta, id_poglavja, id_alineje):
    ...
    ucilnica.seznam[id_predmeta].seznam[id_poglavja].seznam[
        id_alineje
    ].spremeni_stanje_opravljenega(False)
    ucilnica.shrani_v_datoteko(ime_datoteke)
    path = "/" + str(id_predmeta) + "/" + str(id_poglavja) + "/"
    bottle.redirect(path)
...
def izbrisi_alinejo(id_predmeta, id_poglavja, id_alineje):
    ...
    ime_datoteke = ime_uporabnikove_datoteke(uporabnisko_ime)
    ucilnica = ucilnica_trenutnega_uporabnika()
    ucilnica.seznam[id_predmeta].seznam[id_poglavja].seznam.remove(
        ucilnica.seznam[id_predmeta].seznam[id_poglavja].seznam[id_alineje]
    )
    ucilnica.shrani_v_datoteko(ime_datoteke)
    path = "/" + str(id_predmeta) + "/" + str(id_poglavja) + "/"
    bottle.redirect(path)
...
def izbrisi_poglavje(id_predmeta, id_poglavja):
    ...
    ime_datoteke = ime_uporabnikove_datoteke(uporabnisko_ime)
    ucilnica = ucilnica_trenutnega_uporabnika()
    ucilnica.seznam[id_predmeta].seznam.remove(
        ucilnica.seznam[id_predmeta].seznam[id_poglavja]
    )
    ucilnica.shrani_v_datoteko(ime_datoteke)
...
)  # če daš na javno spletno stran, potem debug izklopiš
